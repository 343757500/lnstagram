...
import guide.lifecycle.FragmentLifecycleAdapter;
import guide.lifecycle.V4ListenerFragment;
import guide.listener.OnGuideChangedListener;
import guide.listener.OnPageChangedListener;
...
public class Controller {
    ...
    private void addListenerFragment() {
        //fragment监听销毁界面关闭引导层
        if (fragment != null) {
            fragment.setFragmentLifecycle(new FragmentLifecycleAdapter() {
                @Override
                public void onPause() {
                    GuideLogUtil.i("baseFragment.onPause");
                    remove();
                }
            });
        }
        ...
        if (v4Fragment != null) {
            ...
            v4ListenerFragment.setFragmentLifecycle(new FragmentLifecycleAdapter() {
                ...
                public void onDestroyView() {
                    GuideLogUtil.i("v4ListenerFragment.onDestroyView");
                    remove();
                }
            });
        }
    ...
    private void removeListenerFragment() {
        //隐藏引导层时移除监听fragment
        if (fragment != null) {
            FragmentManager v4Fm = fragment.getChildFragmentManager();
            V4ListenerFragment v4ListenerFragment = (V4ListenerFragment) v4Fm.findFragmentByTag(LISTENER_FRAGMENT);
            if (v4ListenerFragment != null) {
