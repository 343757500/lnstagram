...
import skinlibrary.SkinLoaderListener;
import skinlibrary.utils.ResourcesCompat;
import skinlibrary.utils.SkinFileUtils;
import skinlibrary.utils.SkinLog;
import skinlibrary.utils.TypefaceUtils;
...
//        }
            ...
            protected Resources doInBackground(String... params) {
                try {
                    if (params.length == 1) {
                        ...
                        SkinLog.d("skinPackagePath:" + skinPkgPath);
    ...
    public String getString(int resId) {
        ...
        String resName = context.getResources().getResourceEntryName(resId);
        String turString = StringLoader.getString(resName);
        if (null == turString || "".equals(turString)) {
            turString = originString;
        }
        return turString;
    }
    ...
    public String getString(int resId, Object... formatArgs) {
        ...
        String turString = "";
        try {
            originString = context.getResources().getString(resId, formatArgs);
            if (!SkinConfig.isCanChangeLanguage()) {
                return originString;
            }

            String resName = context.getResources().getResourceEntryName(resId);
            String temp = StringLoader.getString(resName);
            if (null == temp || "".equals(temp)) {
                turString = originString;
            } else {
                turString = String.format(temp, formatArgs);
            }
